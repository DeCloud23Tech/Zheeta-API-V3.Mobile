...
class DurationSelection extends StatefulWidget {
  ...
  const DurationSelection(this.selectedDuration, this.durations, this.onDurationSelected, {super.key});
...
class _DurationSelectionState extends State<DurationSelection> {
  ...
  void initState() {
    super.initState();
    _currentSelectedDuration = widget.selectedDuration; // Initialize the local variable
  }

  @override
  Widget build(BuildContext context) {
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceAround, // Center items and give equal spacing
      children: widget.durations.map((duration) {
        final isSelected = _currentSelectedDuration == duration; // Use the local variable
