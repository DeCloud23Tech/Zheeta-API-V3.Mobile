...
class AgreementService {
  ...
  static bool _isAgreementDialogActive = false;
  ...
  Future<void> checkAndShowAgreementDialog(BuildContext context) async {
    if (!context.mounted || _isAgreementDialogActive) return;

    final bool agreementShownForSession =
        await _storage.getBool(_agreementShownKey) ?? false;

    if (!agreementShownForSession && context.mounted) {
      _isAgreementDialogActive = true;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!context.mounted) {
          _isAgreementDialogActive = false;
          return;
        }
